...
def getTokenType(nextToken):
	...
	elif(nextToken[0].isalpha()): # if a variable starts with a letter and followed by either digits or letters then its an 'id' and doesn't fulfill above conditions
		for char in nextToken:
			if (not (char.isalpha() or char.isdigit() or char == "_")):  #If the variable contains something apart from letters digits and underscores, return error
				return 'error'
		return 'id'
